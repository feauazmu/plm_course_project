...
cnfMtr <- confusionMatrix(pred, valDF$classe)

cnfMtr

```

As can be seen above the model gives an accuracy of `r round(cnfMtr$overall[['Accuracy']]*100, 3)`\% on the validation data.
...
data.frame(prediction=predTest, row.names=testing$X)
```
